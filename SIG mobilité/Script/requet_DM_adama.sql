...
GROUP BY  e.libelle,  c.nom_com, a.stop_id;
...
WHERE e.libelle ILIKE 'ÉCOLE MATERNELLE'
GROUP BY c.nom_com
ORDER BY nombre_ECOLE DESC,
--------- accesibilité au une PHARMACIE par commune 
SELECT 
    c.nom_com, 
    COUNT(e.idequipe) AS nb_PHARMACIE
FROM commune c
JOIN equipement e ON ST_Intersects(e.geom, c.geom)
WHERE e.libelle ILIKE 'PHARMACIE'
GROUP BY c.nom_com
ORDER BY nb_PHARMACIE DESC;
-----Communes avec population (carreaux) sans pharmacie
SELECT 
    c.nom_com, 
    SUM(car.ind) AS pop_sans_pharmacie
FROM commune c
